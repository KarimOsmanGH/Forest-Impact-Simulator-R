...
if (file.exists(sample_file)) {
  cat("**Sample data file found!** Loading data...\n\n")
...
if (exists("tree_data") && nrow(tree_data) > 0) {
...
json_output <- list(
  ...
  environmentalData = list(
    soil = list(
      carbon = as.numeric(env_data$Value[env_data$Metric == "Soil Carbon (g/kg)"]),
      pH = as.numeric(env_data$Value[env_data$Metric == "Soil pH"])
    ),
    climate = list(
      temperature = as.numeric(env_data$Value[env_data$Metric == "Temperature (°C)"]),
...
1. **Data Input**: Either upload a CSV file with forest data or modify the manual input section with your data.
...
5. **Customization**: Modify the environmental metrics, tree types, or calculation methods as needed for your specific use case.
