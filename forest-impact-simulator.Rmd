...
# Function to parse structured CSV
parse_structured_csv <- function(file_path) {
  ...
  return(list(metadata = metadata, tree_data = tree_data, env_data = env_data, planting_data = planting_data))
}

# Check for CSV files
structured_file <- "sample-forest-planting-data.csv"
standard_file <- "sample-forest-data-standard.csv"
has_csv_data <- FALSE
csv_format <- "none"

if (file.exists(structured_file)) {
  cat("**Structured CSV file found!** Loading data...\n\n")
  csv_data <- parse_structured_csv(structured_file)
  has_csv_data <- TRUE
  csv_format <- "structured"
  
  # Display loaded data
  cat("**Metadata:**\n")
  print(csv_data$metadata)
  
  cat("\n**Tree Data:**\n")
  print(csv_data$tree_data)
  
  cat("\n**Environmental Data:**\n")
  print(csv_data$env_data)
  
  cat("\n**Planting Data:**\n")
  print(csv_data$planting_data)
  
} else if (file.exists(standard_file)) {
  cat("**Standard CSV file found!** Loading data...\n\n")
  standard_data <- read.csv(standard_file, stringsAsFactors = FALSE)
  has_csv_data <- TRUE
  csv_format <- "standard"
  
  # Display loaded data
  cat("**Standard CSV Data:**\n")
  print(standard_data)
...
if (mode == "planting") {
  cat("🌱 **Planting Mode**: Calculating positive carbon sequestration from new forest growth\n")
} else {
  cat("🌲 **Clear-cutting Mode**: Calculating carbon loss from forest removal\n")
}
```
...
# Process the data based on format and mode
if (has_csv_data && csv_format == "structured") {
  # Use structured data from CSV file
  current_data <- csv_data$tree_data
  area_ha <- as.numeric(csv_data$planting_data$Value[csv_data$planting_data$Metric == "Area (hectares)"])
  simulation_years <- as.numeric(csv_data$metadata$Value[csv_data$metadata$Parameter == "Simulation Years"])
  latitude <- as.numeric(csv_data$metadata$Value[csv_data$metadata$Parameter == "Latitude"])
  longitude <- as.numeric(csv_data$metadata$Value[csv_data$metadata$Parameter == "Longitude"])
  
  # Extract environmental values
  soil_carbon <- as.numeric(csv_data$env_data$Value[csv_data$env_data$Metric == "Soil Carbon (g/kg)"])
  soil_ph <- as.numeric(csv_data$env_data$Value[csv_data$env_data$Metric == "Soil pH"])
  temperature <- as.numeric(csv_data$env_data$Value[csv_data$env_data$Metric == "Temperature (°C)"])
  precipitation <- as.numeric(csv_data$env_data$Value[csv_data$env_data$Metric == "Precipitation (mm)"])
...
} else if (has_csv_data && csv_format == "standard") {
  # Use standard CSV data
  land_data <- standard_data
  
  # Extract location and simulation data from first row
  location_data <- data.frame(
    latitude = land_data$latitude[1],
    longitude = land_data$longitude[1],
    simulation_years = land_data$simulation_years[1]
  )
  
  # Create environmental data from first row
  env_data_structured <- data.frame(
    soil_carbon = land_data$soil_carbon[1],
    soil_ph = land_data$soil_ph[1],
    temperature = land_data$temperature[1],
    precipitation = land_data$precipitation[1]
  )
...
1. **Data Input**: The notebook automatically detects and supports multiple formats:
   - **Standard CSV** (Recommended): Simple table format in `sample-forest-data-standard.csv`
   - **Structured CSV**: Complex format from web app in `sample-forest-planting-data.csv`
   - **Manual Input**: Modify the manual input section with your data
...
5. **CSV Formats Supported**:
   
   **Standard CSV Format** (Easy to use):
   ```csv
